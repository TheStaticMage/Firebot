...
import { EventSubChannelBitsUsePowerUp } from './EventSubChannelBitsUsePowerUp';
...
export class EventSubChannelBitsUseEvent extends DataObject<EventSubChannelBitsUseEventData> {
	...
	get powerUp(): EventSubChannelBitsUsePowerUp | null {
        ...
        if (this[rawDataSymbol].powerUpType === 'message_effect') {
            this[rawDataSymbol].power_up = {
                type: this[rawDataSymbol].powerUpType,
                emote: null,
                message_effect_id: 'test_message_effect_id',
            }
        }

		if (this[rawDataSymbol].powerUpType === 'celebration') {
            this[rawDataSymbol].power_up = {
                type: this[rawDataSymbol].powerUpType,
                emote: null,
		...
		if (this[rawDataSymbol].powerUpType === 'gigantify_an_emote') {
            this[rawDataSymbol].power_up = {
                type: this[rawDataSymbol].powerUpType,
                emote: { 'id': 'test_emote_id', 'name': 'test_emote_name' },
                message_effect_id: null,
            }
        }
